...
function Login(props) {
    ...
    if(email === 'go@go.com' && password === 'qwerty'){
        return (
                      ...
                      <CardBody>
                        <form onSubmit={handleSubmit}>
                          ...
                          <ButtonGroup className="w-100 mt-3">
                            <Button color='primary' type="submit">КИР</Button>
                          <Button type="reset" onClick={()=>setModalIsOpen(false)}>КИРБЕСЕН КЕТ</Button>
                          </ButtonGroup>
                          {modalIsOpen===false ? <Redirect to="/"/>:<Redirect to="/auth/login"/>}
                          <p className="text-center mt-3"> Ты не постучился? {' '} <Link to="/auth/register">Постучиться</Link></p>
